...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const isBlog = typeof location !== 'undefined' && location.pathname.indexOf('/blog/') !== -1;
  ...
  style.textContent = `
    ...
    .wa-nav-btn{background:#25D366;color:#fff;padding:.5rem 1rem;border-radius:.5rem;display:inline-flex;align-items:center;gap:.5rem;white-space:nowrap;flex:0 0 auto}
    .wa-nav-btn:hover{background:#1ebe57}
    /* Ensure hidden mobile menu never overlays layout when inactive */
  ...
  // Add WhatsApp CTA to desktop navbar (skip on blog pages to avoid header crowding)
  if (!isBlog) {
  ...
  // Add WhatsApp link to mobile menu (skip on blog pages to avoid accidental overlay)
  if (!isBlog) {
